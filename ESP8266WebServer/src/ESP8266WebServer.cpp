...
String ESP8266WebServer::cookie(String name) {
...
String ESP8266WebServer::cookieName(int i) {
  if (i < _cookieKeysCount)
    return _currentCookies[i].key;
  return "";
}

int ESP8266WebServer::cookies() {
  return _cookieKeysCount;
}

bool ESP8266WebServer::hasCookie(String name) {
  for (int i = 0; i < _cookieKeysCount; ++i) {
