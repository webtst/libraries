...
bool ESP8266WebServer::_parseRequest(WiFiClient& client) {
  ...
  for (int i = 0; i < _cookieKeysCount; ++i) {
    _currentCookies[i].value =String();
  }
...
bool ESP8266WebServer::_collectCookie(const char* cookieName, const char* cookieValue) {
  for (int i = 0; i < _cookieKeysCount; i++) {
    if (_currentCookies[i].key.equalsIgnoreCase(cookieName)) {
            _currentCookies[i].value=cookieValue;
            return true;
        }
  }
  return false;
...
void ESP8266WebServer::_parseCookies(String cook){
  cook += "; ";
  String tmp;
  int hv = cook.indexOf("; ");
  int v = 0;
#ifdef DEBUG_ESP_HTTP_SERVER
DEBUG_OUTPUT.println("Cookies:");
#endif
  while (hv != -1){
    tmp = cook.substring(0,hv);
    v = tmp.indexOf('=');
    _collectCookie(tmp.substring(0,v).c_str(),tmp.substring(v+1,tmp.length()).c_str());
#ifdef DEBUG_ESP_HTTP_SERVER
DEBUG_OUTPUT.print(tmp.substring(0,v));
DEBUG_OUTPUT.print("=");
DEBUG_OUTPUT.println(tmp.substring(v+1,tmp.length()));
#endif
    cook = cook.substring(hv+2,cook.length());
    hv = cook.indexOf("; ");
  };
}
